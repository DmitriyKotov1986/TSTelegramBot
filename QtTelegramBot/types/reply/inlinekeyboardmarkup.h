...
{
public:
    InlineKeyboardMarkup(InlineKeyboardButtons buttons, quint8 RowsCount = 1)
        : GenericReply(false),
          buttons(buttons),
          RowsCount(RowsCount)
    ...
    const quint8 RowsCount;

    virtual QString serialize() const {
        QJsonObject o = QJsonObject();
        QJsonArray keyboardButtons = QJsonArray();
        auto ButtonIter = buttons.begin();
        while (ButtonIter != buttons.end()) {
            QJsonArray array;
            for (quint8 i = 0; (i < RowsCount) && (ButtonIter != buttons.end()); ++i) {
                InlineKeyboardButton CurrentButton = *ButtonIter;
                array.append(CurrentButton.toJsonObject());
                ++ButtonIter;
            }
